...
class GoogleAnalyticsClient:
    ...
    def __init__(self, storage=None, redirect_uri=None):
        self.FLOW = flow_from_clientsecrets(
            client_id=getattr(settings, 'ANALYTICS_OAUTH_CLIENT_ID', ''),
            client_secret=getattr(
                settings, 'ANALYTICS_OAUTH_CLIENT_SECRET', ''),
...
class CredentialWidget(Widget):
    ...
    def render(self, name, value, attrs=None):
        if value and len(value) > 0:
            link = '<a href="%s">%s</a>' % (
                reverse('jet-dashboard:google-analytics-revoke',
                        kwargs={'pk': self.module.model.pk}),
                force_text(_('Revoke access'))
            )
        else:
            link = '<a href="%s">%s</a>' % (
                reverse('jet-dashboard:google-analytics-grant',
                        kwargs={'pk': self.module.model.pk}),
...
class GoogleAnalyticsBase(DashboardModule):
    ...
    def counter_attached(self):
        if self.credential is None:
            self.error = mark_safe(_('Please <a href="%s">attach Google account and choose Google Analytics counter</a> to start using widget') %
                                   reverse('jet-dashboard:update_module', kwargs={'pk': self.model.pk}))
            return False
        elif self.counter is None:
            self.error = mark_safe(_('Please <a href="%s">select Google Analytics counter</a> to start using widget') %
                                   reverse('jet-dashboard:update_module', kwargs={'pk': self.model.pk}))
            return False
        else:
            return True
    ...
    def api_ga(self, group=None):
        if self.counter_attached():
            ...
            try:
                client = GoogleAnalyticsClient(self.storage)
                result, exception = client.api_ga(
...
class GoogleAnalyticsVisitorsTotals(GoogleAnalyticsBase):
    ...
    def init_with_context(self, context):
        ...
        if result is not None:
            try:
                self.children.append({'title': _('users'), 'value': result[
                                     'totalsForAllResults']['ga:users']})
                self.children.append({'title': _('sessions'), 'value': result[
                                     'totalsForAllResults']['ga:sessions']})
                self.children.append({'title': _('views'), 'value': result[
            ...
            try:
                for data in result['rows']:
                    ...
                    self.children.append(
                        (date, row_data[self.show.replace(':', '_')]))
            except KeyError:
                self.error = _('Bad server response')
