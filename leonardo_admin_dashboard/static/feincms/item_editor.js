(function($){
    ...
    function save_content_type_selects() {
        $('#main>.panel').each(function() {
            SELECTS[this.id.replace(/_body$/, '')] = $("select[name=order-machine-add-select]", this).clone().removeAttr("name").removeAttr('class');
        });
    }
    ...
    $(document).ready(function($){
        ...
        $("input.order-machine-add-button").click(function(){
            var select_content = $(this).closest('div').find('select');
            var modvar = select_content.val();
